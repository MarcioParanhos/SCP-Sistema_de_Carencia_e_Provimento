...
@section('content')
    ...
    <div id="aposentadoria_info" class="d-flex justify-content-between mb-4">
        ...
        <div id="aposentadoria_info_content" class="d-flex justify-content-between" style="width: 50%;">
            <div class="col-md-6">
                <table class="table-bordered">
                    <tr>
                        <td colspan="2" class="pl-2 text-center bg-primary text-white subheader"><b>TOTAL GERAL -
                                ENCAMINHAMENTOS</b></td>
                    </tr>
                    <tr>
            ...
            <div class="col-md-6">
                <table class="table-bordered">
                    <tr>
                        <td colspan="2" class="pl-2 text-center bg-primary text-white subheader"><b>PENDENCIAS E
                                AÇÕES</b></td>
                    </tr>
                    <tr>
                        <td class="pl-2 subheader"><b>Encaminhamentos com inconsistência</b></td>
                        <td style="width: 20%;" class="text-center">
                            <b>{{ $quantidadeRegistrosError - $quantidadeRegistrosErrorOK }}</b></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td style="width: 20%;" class="text-center">
                            <b>{{ $quantidadeRegistros - $quantidadeRegistrosPCH - ($quantidadeRegistrosError - $quantidadeRegistrosErrorOK) - $quantidadeRegistrosDataNula - $quantidadeRegistrosAtrasados }}</b>
                        </td>
                    </tr>
                </table>
            </div>
        ...
        <div class="mb-2 ">
            <a id="active_filters" data-toggle="tooltip" data-placement="top" title="Filtros Personalizaveis"
                class="mb-2 btn bg-primary text-white" onclick="active_filters()">
            ...
            @if (Auth::user()->profile === 'cad_tecnico' || Auth::user()->profile === 'administrador')
                <button type="button" class="mb-2 btn bg-primary text-white" data-toggle="modal"
                    data-target="#addNewProcess">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            ...
            @endif
            <a class="mb-2 btn bg-primary text-white" data-toggle="tooltip" data-placement="top" title="Download em Excel"
                target="_blank" href="{{ route('aposentadorias.excel') }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
    ...
    <form id="active_form" class="border shadow bg-light rounded pt-3 pl-3 pr-3"
        action="{{ route('provimento_efetivo.showByForm') }}" method="post" hidden>
        @csrf
        <div class="form-row">
            <div class="col-md-1">
                <div class="form-group_disciplina">
                    <label class="control-label" for="search_nte_provimento_efetivos">NTE</label>
                    <select name="search_nte_provimento_efetivos" id="nte_seacrh"
            ...
            <div class="col-md-4">
                <div class="form-group_disciplina">
                    ...
                    <select name="search_municipio_provimento_efetivos" id="municipio_search"
                        class="form-control form-control-sm select2">
            ...
            <div class="col-md-5">
                <div class="form-group_disciplina">
                    ...
                    <select name="search_uee_provimento_efetivos" id="search_uee"
                        class="form-control form-control-sm select2">
                        <option></option>
                    </select>
                </div>
            ...
            <div class="col-md-2">
                <div class="form-group_disciplina">
                    ...
                    <input value="" name="search_codigo_unidade_escolar_efetivo" id="search_codigo_unidade_escolar"
        ...
        <div class="form-row mb-4">
            <div class="col-md-2">
                <div class="form-group_disciplina">
                    ...
                    <input value="" name="search_cpf_servidor_efetivo" id="search_matricula_servidor"
            ...
            <div class="col-md-2">
                <div class="form-group_disciplina">
                    ...
                    <select name="search_pch_efetivo" id="search_pch_efetivo"
                        class="form-control form-control-sm select2">
                        <option></option>
            ...
            <div class="col-md-2">
                <div class="form-group_disciplina">
                    ...
                    <select name="search_assuncao_efetivo" id="search_assuncao_efetivo"
                        class="form-control form-control-sm select2">
                        <option></option>
    ...
    <div class="table-responsive">
            ...
            <tbody>
                ...
                @foreach ($groups as $ingressoId => $group)
                    @php
                        ...
                        $matutino_sum = $group->sum(function ($r) {
                            return intval($r->matutino ?? 0);
                        });
                        $vespertino_sum = $group->sum(function ($r) {
                            return intval($r->vespertino ?? 0);
                        });
                        $noturno_sum = $group->sum(function ($r) {
                            return intval($r->noturno ?? 0);
                        });
                        $has_any_assuncao = $group->contains(function ($r) {
                            return !empty($r->data_assuncao) && $r->data_assuncao !== '0000-00-00';
                        });
                        $any_without_assuncao = $group->contains(function ($r) {
                            return empty($r->data_assuncao) || $r->data_assuncao === '0000-00-00';
                        });
                        $any_without_assuncao_overdue = $group->contains(function ($r) {
                            if (empty($r->data_assuncao) || $r->data_assuncao === '0000-00-00') {
                                if (!$r->data_encaminhamento) {
                                    return false;
                                }
                                try {
                                    $days = \Carbon\Carbon::parse($r->data_encaminhamento)->diffInDays(
                                        \Carbon\Carbon::now(),
                                    );
                                    return $days >= 2;
                                } catch (\Exception $e) {
                        ...
                        $all_pch_ok = $group->every(function ($r) {
                            return $r->pch === 'OK';
                        });
                        $any_inconsistency = $group->contains(function ($r) {
                            return $r->server_1_situation == 2 || $r->server_2_situation == 2;
                        });
                    @endphp
                    ...
                    <tr>
                        ...
                        @endif
                        @php $srvName = $srv->name ?? $srv->nome ?? null; @endphp
                        <td class="text-center">{{ $srvName ?? '— (sem nome) (id:' . $first->ingresso_candidato_id . ')' }}
                        </td>
                        ...
                        <td class="text-center d-flex align-items-center justify-content-center">
                            <a href="/provimento/efetivo/detail/{{ $ingressoId }}" class=""><button
                                    id="" type="submit" class="btn btn-primary">
                            ...
                            @if (Auth::user()->profile === 'cpm_tecnico' ||
                                ...
                                @if ($first->pch != 'OK' || Auth::user()->profile === 'administrador')
                                    <a data-toggle="tooltip" data-placement="top" title="Excluir" title="Excluir"
                                        id="" onclick="destroyProvimentoEfetivo('{{ $first->id }}')"
        ...
        <tfoot>
            <tr>
                <td colspan="100%">
                    <h6>SUMÁRIO</h6>
                    <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 10px; padding: 10px; border-radius: 8px; background-color: #ffffff;"
                        class="border mt-2">
                        ...
                        <div style="display: flex; align-items: center; gap: 8px;" class="text-warning">
                            ...
                            <span class="subheader"><strong>ENCAMINHAMENTOS SEM ASSUNÇÃO, MAS DENTRO DO
                                    PRAZO</strong></span>
                        </div>
                        ...
                        <div style="display: flex; align-items: center; gap: 8px;" class="text-success">
                            ...
                            <span class="subheader"><strong>ENCAMINHAMENTOS COM DATA DE ASSUNÇÃO | VALIDAÇÃO DA
                                    CPG</strong></span>
                        </div>
