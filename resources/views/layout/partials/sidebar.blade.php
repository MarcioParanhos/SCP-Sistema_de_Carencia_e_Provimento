<nav class="print-none  sidebar shadow rounded sidebar-offcanvas print-hidden" id="sidebar">
    ...
    @php
        ...
        $isCpm =
            isset($sidebarUser) &&
            isset($sidebarUser->profile_id) &&
            $sidebarUser->profile_id == 1 &&
            isset($sidebarUser->sector_id) &&
            $sidebarUser->sector_id == 2;
        $canIngresso =
            isset($sidebarUser) &&
            isset($sidebarUser->profile_id) &&
            $sidebarUser->profile_id == 1 &&
            isset($sidebarUser->sector_id) &&
            in_array($sidebarUser->sector_id, [7, 2]);
    @endphp
    ...
    @if (
        !(isset($sidebarUser) &&
            ...
            $sidebarUser->profile_id == 1 &&
            isset($sidebarUser->sector_id) &&
            $sidebarUser->sector_id == 7
        ))
        <ul class="nav">

            @if (
                $isCpm ||
                    (Auth::user()->profile != 'cad_tecnico' &&
                ...
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('ingresso.dashboard') }}">
                        ...
                        <span class="menu-title">Processo de Ingresso</span>
                    </a>
                </li>
                <li class="nav-item">
                    ...
                    <div class="collapse subheader" id="ui-basic">
                        <ul class="nav flex-column sub-menu">
                            @if ($isCpm || Auth::user()->profile === 'cpg_tecnico' || Auth::user()->profile === 'administrador')
                ...
                <li class="nav-item">
                    ...
                    <div class="collapse subheader" id="form-elements">
                        <ul class="nav flex-column sub-menu">
                            @if ($isCpm || in_array(Auth::user()->profile_id, ['1', '4', '2']))
            ...
            @endif
            @if ($isCpm || Auth::user()->profile != 'cgi_tecnico')
                <li class="nav-item">
    ...
    @else
        <ul class="nav">
            ...
            @endif
            <li class="nav-item">
                <a class="nav-link" href="/carencias/all_carencias">
                    <i class="icon-square-plus menu-icon"></i>
                    <span class="menu-title">CARÊNCIA</span>
                </a>
            </li>
