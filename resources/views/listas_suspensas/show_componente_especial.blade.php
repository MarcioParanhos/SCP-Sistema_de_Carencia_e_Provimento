...
@section('title', 'SCP - Componente Especial')
...
<div class="bg-primary card text-white card_title">
    <h3 class=" title_show_carencias ">COMPONENTE ESPECIAL</h3>
</div>
...
<div class="table-responsive">
    <table id="consultarCarencias" class=" table table-sm table-hover table-bordered">
        <caption class="mt-2">Configuração de Componentes Especiais</caption>
        <thead class="">
            <tr class="text-center" style="vertical-align: middle;">
                <th class="bg-primary text-white" scope="col">ID</th>
                <th class="bg-primary text-white" scope="col">COMPONENTE ESPECIAL</th>
                <th class="bg-primary text-white" scope="col">AÇÃO</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($componentes_especiais as $componente_especial)
            <tr>
                <td class="text-center">{{ $componente_especial->id }}</td>
                <td class="text-center">{{ $componente_especial->nome }}</td>
                <td class="text-center">
                    <!-- <a title="Editar" id="" class="ml-1 btn-show-carência btn btn-sm btn-primary"><i class="fa-solid fa-pencil"></i></a> -->
                    <a title="Excluir" id="" onclick="destroyComponenteEspecial('{{ $componente_especial -> id }}')" class="ml-1 btn-show-carência btn btn-sm btn-danger"><i class="ti-trash"></i></a>
                </td>
            </tr>
            @endforeach
        </tbody>
