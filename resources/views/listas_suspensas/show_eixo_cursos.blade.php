...
<div class="bg-primary card text-white card_title">
    <h3 class=" title_show_carencias ">CURSOS E EIXOS</h3>
</div>
<div class="print-btn mb-4">
    ...
    <a class="mb-2 btn bg-primary text-white" href=""><i class="fa-solid fa-arrow-left"></i> VOLTAR</a>
</div>

<div class="table-responsive">
    <table id="consultarCarencias" class=" table table-sm table-hover table-bordered">
        <caption class="mt-2">Configuração de Cursos e Eixos</caption>
        <thead class="">
            <tr class="text-center" style="vertical-align: middle;">
                <th class="bg-primary text-white" scope="col">ID</th>
                <th class="bg-primary text-white" scope="col">CURSO</th>
                <th class="bg-primary text-white" scope="col">EIXO</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">{{ $curso->eixo }}</td>
                <td class="text-center">
                    <!-- <a title="Editar" id="" class="ml-1 btn-show-carência btn btn-sm btn-primary"><i class="fa-solid fa-pencil"></i></a> -->
                    <a title="Excluir" id="" onclick="destroyCurso('{{ $curso -> id }}')" class="ml-1 btn-show-carência btn btn-sm btn-danger"><i class="ti-trash"></i></a>
                </td>
            </tr>
            @endforeach
        </tbody>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addNewDisciplina">ADICIONAR NOVO CURSO</h5>
            ...
            <div class="modal-body">
                <div class="col-md-12">
                    <form class="forms-sample" id="InsertArea" action="{{ route('cursos.create') }}" method="POST">
                        @csrf
                        <div class="form-group row">
                            <div class="col-md-8">
                                <div class="form-group_disciplina">
                                    <label class="control-label" for="curso">NOME</label>
                                    <input value="" name="curso" id="curso" type="text" class="form-control form-control-sm">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group_disciplina">
                                    <label class="control-label" for="eixo">EIXO</label>
                                    <select name="eixo" id="eixo" class="form-control form-control-sm">
                                        <option>Selecione....</option>
                                        @foreach ($eixos as $eixo)
                                        <option value="{{ $eixo->eixo }}">{{ $eixo->eixo }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
        ...
        if (session_message) {
            if (session_message.value === "error") {
                Swal.fire({
                    icon: 'error',
                    title: 'Atenção!',
                    text: 'A exclusão do Curso não é viável devido à existência de carências e provimentos a ela.',
                })
