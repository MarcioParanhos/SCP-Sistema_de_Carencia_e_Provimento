...
@section('content')
    ...
    <div class="form_content mb-0">
        <div class="mb-2 d-flex justify-content-end " style="gap: 3px;">
            ...
            <a class="mb-2 btn bg-primary text-white" target="_blank" href="/relatorio/carencia" data-toggle="tooltip"
        ...
        <form id="active_form" class="border shadow bg-light rounded pt-3 pl-3 pr-3 transition-hidden"
            action="/carencia/filter_carencias" method="post" hidden>
            @csrf
            <div class="form-row">
                <div class="col-md-1">
                    ...
                    <div class="form-group_disciplina">
                        ...
                        <select name="search_codigo[]" class="form-control form-control-sm" id="search_codigo" multiple>
                            @foreach ($unidadesEscolares as $unidadeEscolar)
                                <option value="{{ $unidadeEscolar->cod_unidade }}">{{ $unidadeEscolar->cod_unidade }}
                                </option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="form-row d-flex">
                <div class="col-md-2">
                    <div class="form-group_disciplina">
                        ...
                        <select name="search_situacao_homologacao" id="search_situacao_homologacao"
                ...
                <div class="col-md-2">
                    <div class="form-group_disciplina">
                        ...
                        <select name="search_tipo_vaga" id="search_tipo_vaga"
                            class="form-control form-control-sm select2">
                            <option></option>
                ...
                <div class="col-md-2">
                    <div class="form-group_disciplina">
                        ...
                        <input name="search_matricula_servidor" id="search_matricula_servidor" type="number"
                ...
                <div class="">
                    <ul class="ks-cboxtags">
                        <li><input name="check" type="checkbox" id="checkbox50"><label for="checkbox50">LICENÇAS
                                VENCIDAS</label></li>
                    </ul>
                </div>
            </div>
        ...
        @if (Auth::user()->profile === 'administrador' || Auth::user()->profile === 'cpm_tecnico' || Auth::user()->profile === 'cpm_coordenador')
            <div id="buttons" class="buttons mb-4">
                <button class="ml-2 button" type="button" id="reservarBtn">
                    <span class="button__text subheader">Reservar</span>
                    <span class="button__icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-copy-check">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                    ...
                    <tr @if (!empty($carencia->vagaReserva)) class="table-warning" @endif>
                        @if (!$carencia->vagaReserva)
                            <td class="text-center">
                                <input type="checkbox" class="row-checkbox" value="{{ $carencia->id }}">
                            </td>
                        @else
                        ...
                        <td>{{ $carencia->uee->municipio }}</td>   
                        <td>{{ $carencia->uee->unidade_escolar }}</td>
                        <td class="text-center">{{ $carencia->uee->cod_unidade }}</td>
                        @if ($carencia->tipo_carencia === 'Real')
                            <td data-toggle="modal" data-target="#modalInfo" class="text-center cursor-pointer"><span
                                    class="tipo_carencia">R</span></td>
                        @endif
                        @if ($carencia->tipo_carencia === 'Temp')
                            <td data-toggle="modal" data-target="#modalInfo" class="text-center cursor-pointer"><span
                                    class="tipo_carencia">T</span></td>
                        @endif
                        @if ($carencia->uee->situacao === 'HOMOLOGADA')
                            <td class="text-success text-center"><strong>SIM</strong></td>
                        @endif
                        @if ($carencia->uee->situacao === 'PENDENTE')
                        ...
                        <td class="d-flex text-center">
                            <a data-toggle="tooltip" data-placement="top" title="Detalhes" title="Detalhar"
                                href="/detalhar_carencia/{{ $carencia->id }}"><button id=""
                            ...
                            @if (Auth::user()->profile === 'cpg_tecnico' || Auth::user()->profile === 'administrador')
                                @if (!$carencia->vagaReserva)
                                    @if (Auth::user()->profile === 'cpg_tecnico' || Auth::user()->profile === 'administrador')
                                        ...
                                        @elseif ($carencia->uee->situacao === 'PENDENTE')
                                            <a data-toggle="tooltip" data-placement="top" title="Excluir"
                                                title="Excluir" id="" onclick="destroy('{{ $carencia->id }}')"
                                                class="ml-1 btn btn-danger">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                            ...
                            @if (session('ano_ref') == $ano_atual || Auth::user()->profile === 'administrador')
                                @if (Auth::user()->profile === 'cpm_tecnico' || Auth::user()->profile === 'administrador')
                                    @if ($carencia->hml === 'SIM')
                                        @if (!$carencia->vagaReserva)
                                            {{-- <a data-toggle="tooltip" data-placement="top" title="Prover" title="Prover"
                                                id=""
                                                href="/prover/{{ $carencia->id }}/{{ $carencia->cod_ue }}"
                                    ...
                                    @elseif ($carencia->hml === 'NÃO')
                                        {{-- <a data-toggle="tooltip" data-placement="top" title="Prover" title="Prover"
                                            id="" href="/prover/{{ $carencia->id }}/{{ $carencia->cod_ue }}"
                                            class="ml-1 btn btn-info" hidden>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                class="icon icon-tabler icons-tabler-outline icon-tabler-replace">
                    ...
                    <div class="">
                        ...
                        <div class="pt-2">
                            ...
                            <div class="mt-3 d-flex align-items-center">
                                <span class="tipo_carencia mr-1"
                                    style="display: inline-block; width: 80px; height: 30px; background-color: #ffeeb8;"></span>
                        ...
                        <div class="col-md-10">
                            <div class="d-flex">
                                <button type="button" style="width: 150px;"
                                    class="btn btn-success flex-fill mx-1 py-2 subheader" id="selectAllBtn">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <button type="button" style="width: 150px;"
                                    class="btn btn-danger flex-fill mx-1 py-2 subheader" id="deselectAllBtn">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <form id="exportForm">
                        <div class="row">
                            ...
                            $colunasDisponiveis = [
                                ...
                                'noturno' => 'NOT',
                                'total' => 'Total',
                                'inicio_vaga' => 'Início da Vaga',
                                'usuario' => 'Usuário',
                                'created_at' => 'Data de Lançamento',
                            ];
                            
                            // Divide a lista em 3 partes iguais
                            $total = count($colunasDisponiveis);
                            $porColuna = ceil($total / 3);
                            $i = 0;
                            
                            echo '<div class="col-md-4 pr-2">';
                            foreach ($colunasDisponiveis as $key => $label) {
                                if ($i > 0 && $i % $porColuna == 0) {
                                    echo '</div><div class="col-md-4 px-2">';
                                }
...
@push('scripts')
    <script src="{{ asset('js/pages/carencias.js') }}" defer></script>
