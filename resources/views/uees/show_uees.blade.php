...
@section('content')
    ...
    <div class="form_content mb-0">
        <div class="mb-2 print-btn">
            ...
            <a class="mb-2 btn bg-primary text-white" target="_blank" href="{{ route('uees.excel') }}" data-toggle="tooltip"
        ...
        <form id="active_form" class="border shadow bg-light rounded pt-3 pl-3 pr-3" action="{{ route('uees.showByForm') }}"
            method="post" hidden>
            @csrf
                ...
                <div class="form-row col-md-12">
                    ...
                    <div class="col-md-2">
                        <div class="form-group_disciplina">
                            ...
                            <select name="search_etapa_pch" id="search_etapa_pch"
                                class="form-control form-control-sm select2">
                                <option></option>
                    ...
                    <div class="col-md-2">
                        <div class="form-group_disciplina">
                            ...
                            <select name="search_carencia" id="search_carencia"
                                class="form-control form-control-sm select2">
                                <option></option>
                    ...
                    <div class="col-md-3">
                        <div class="form-group_disciplina">
                            ...
                            <select name="search_tipologia[]" class="form-control form-control-sm" id="search_disciplina"
                                multiple>
                                <option value="SEDE">SEDE</option>
                                <option value="ANEXO">ANEXO</option>
                                <option value="CEMIT">CEMIT</option>
                ...
                <table class="mt-0 table table-sm table-bordered">
                    ...
                    <tr>
                        ...
                        <th class="text-center" scope="row">
                            {{ $totalUnits3Pch + $totalUnits2Pch + $totalUnits + $totalUnits4Pch }}</th>
                    </tr>
                    </tbody>
                </table>
            </div>
                ...
                <table class="mt-0 table table-sm table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th style="vertical-align: middle; width: 25%;" class="text-center" scope="col">
                                CEMIT/EMITEC</th>
                            <th style="vertical-align: middle; width: 25%;" class="text-center" scope="col">TOTAL</th>
                        </tr>
                    </thead>
                    <tr>
                        <th style="vertical-align: middle; width: 25%;" class="text-center" scope="row">
                            {{ $totalUnitsSedes }}</th>
                        <th style="vertical-align: middle; width: 25%;" class="text-center" scope="row">
                            {{ $totalUnitsAnexos }}</th>
                        <th style="vertical-align: middle; width: 25%;" class="text-center" scope="row">
                            {{ $totalUnitsCemits }}</th>
                        <th style="vertical-align: middle; width: 25%;" class="text-center" scope="row">
                            {{ $totalUnitsSedes + $totalUnitsAnexos + $totalUnitsCemits }}</th>
                    </tr>
                    </tbody>
                </table>
            </div>
    ...
    <div class="mt-3 table-responsive">
        <table id="consultarCarencias" class="table-bordered table-sm table">
            ...
            <tbody>
                @foreach ($uees as $uee)
                    <tr>
                        <th class="text-center" scope="row">{{ $uee->nte < 10 ? '0' . $uee->nte : $uee->nte }}</th>
                        <td>{{ $uee->municipio }}</td>
                        <td>{{ $uee->unidade_escolar }}</td>
                        <td class="text-center">{{ $uee->cod_unidade }}</td>
                        <td class="text-center">
                            <span style="width: 100% !important"
                                class="badge badge-{{ $uee->uo ? 'success' : 'danger' }}">
                                {{ $uee->uo ?? 'S/ Cod.' }}
                            </span>
                        </td>
                                    ...
                                    @foreach (json_decode($uee->categorias) as $categoria)
                                        @if (empty($searchCategoria) || $categoria == $searchCategoria)
                                            @if ($categoria == 'sedeCemit')
                                                <span class="badge badge-pill badge-primary" style="margin: 2px;">Sede /
                                                    CEMIT</span>
                                            @else
                                                <span class="badge badge-pill badge-primary"
                                                    style="margin: 2px;">{{ $categoria }}</span>
                                            @endif
                                        @endif
                                    @endforeach
                                @endif
                        ...
                        @if (Auth::user()->profile === 'administrador' || Auth::user()->profile === 'cpg_tecnico')
                            ...
                            @else
                                <td class="text-center">
                                    <div class="d-flex">
                                        @if ($uee->situacao != 'NTE')
                                            <a title="Detalhar" href="/uees/detail/{{ $uee->id }}"><button
                                                    id="" class="btn-show-carência btn btn-primary"><i
                                                        class="ti-search"></i></button></a>
                                        @endif
                                        @if (Auth::user()->profile === 'administrador')
                                            <a title="Excluir" href="/uee/destroy/{{ $uee->id }}"
                                                class="ml-1 btn-show-carência btn btn-danger"><i class="ti-trash"></i></a>
                                        @endif
                                    </div>
                                </td>
                            @endif
    ...
    <div class="modal fade" id="createUeesMoral" tabindex="-1" role="dialog" aria-labelledby="createUeesMoral"
        aria-hidden="true">
                            ...
                            <div class="row">
                                <div class="col-md-2">
                                    <div class="form-group_disciplina">
                                        ...
                                        <select name="nte_seacrh" id="nteForCreateUee"
                                            class="form-control form-control-sm select2">
                                            <option></option>
                                            <option>1</option>
                                            <option>2</option>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group_disciplina">
                                        ...
                                        <select name="municipio_search" id="municipioForCreateUee"
                                            class="form-control form-control-sm select2">
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group_disciplina">
                                        ...
                                        <input value="" name="unidade_escolar" id="unidade_escolar" type="text"
                                ...
                                <div class="col-md-2">
                                    <div class="form-group_disciplina">
                                        ...
                                        <input value="" name="cod_unidade" id="cod_unidade" type="number"
                                            class="form-control form-control-sm">
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer d-flex justify-content-center text-center flex-row">
                                <button style="width: 30%;" type="button" class="btn btn-danger"
